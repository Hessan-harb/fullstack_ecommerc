    ...
    useEffect(() => {
      const fetchProductsByCategory = async () => {
        ...
        if (categoryName.toLowerCase() === "all") {
          rawQuery = "{ products { id name description inStock category brand prices { amount currency_label currency_symbol } gallery attributes { name type items { displayValue value } } } }";
        } else {
          rawQuery = `{ productsByCategory(category: "${categoryName}") { id name description inStock category brand prices { amount currency_label currency_symbol } gallery attributes { name type items { displayValue value } } } }`;
        }
        ...
        try {
          ...
          setProducts(result.data.productsByCategory || result.data.products);
        } catch (error) {
          ...
          {products.map((product) => (
            <div key={product.id} className="group my-10 flex w-full max-w-xs flex-col overflow-hidden hover:bg-gray-50 items-center p-2">
              <a className="relative flex h-80 w-72 overflow-hidden " >
                <img className="absolute top-0 right-0 h-full w-full object-cover" src={product.gallery[0]} />
                { !product.inStock && (
